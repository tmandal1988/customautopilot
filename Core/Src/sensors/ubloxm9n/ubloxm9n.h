...
class ReadUbloxM9n : public TaskBase {
public:
	ReadUbloxM9n(UART_HandleTypeDef* huart, I2C_HandleTypeDef* hi2c);
	virtual ~ReadUbloxM9n(){
	}

	void Run() override;

	// Sets the new nav pvt frame flag
	static void ReceivedNewNavPvtFrame();

	static ReadUbloxM9n* ubloxm9n_instance_handle_;

private:
	static constexpr uint16_t READ_INTERVAL_MS = 10; // 100Hz
	// Tx and Rx max delay (ms) in polling mode
	static constexpr uint8_t txrx_delay_ms_ = 1;

	// Define default I2C address of M9N
	static constexpr uint8_t UBX_I2C_ADDR 											= 0x42;
	static constexpr uint8_t ACK_NAK_PAYLOAD_LENGTH 								= 2;
	static constexpr uint16_t MAX_BUFF_SIZE 										= 1024;
	...
	I2C_HandleTypeDef* gps_i2c_; // This I2C is used to config GPS
	...
	bool TxI2cUbxPollCmd(const UbxMessage *message, uint32_t wait_time);
	bool RxI2cUbxPollMsgAck(uint8_t *payload_buffer, uint16_t payload_length, uint32_t wait_time);
	bool RxI2cUbxPollMsg(uint8_t *payload_buffer, uint16_t payload_length, uint32_t wait_time);
	bool ResetGps();
