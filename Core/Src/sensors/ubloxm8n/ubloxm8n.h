...
class ReadUbloxM8n : public TaskBase {
public:
	ReadUbloxM8n(UART_HandleTypeDef* huart);
	virtual ~ReadUbloxM8n(){
	}
	...
	static ReadUbloxM8n* ubloxm8n_instance_handle_;

private:
	static constexpr uint16_t READ_INTERVAL_MS = 100; // 100Hz
	// Tx and Rx max delay (ms) in polling mode
	static constexpr uint32_t txrx_delay_ms_ = 500;
...
//	};
	...
	struct PACKED UbloxM8nCfgRst {
		uint16_t nav_bbr_mask;
		uint8_t reset_mode;
		uint8_t reserved;
	};

	struct PACKED UbloxM8nCfgPrt {
		uint8_t port_id;
		uint8_t reserved;
		uint16_t tx_ready;
		uint32_t mode;
	...
	struct PACKED UbloxM8nCfgMsg {
		uint8_t msg_class;
		uint8_t msg_id;
		uint8_t rate[6];
	};

	struct PACKED UbloxM8nCfgNav5 {
		uint16_t mask;
		uint8_t dyn_model;
		uint8_t fix_mode;
		int32_t fixed_alt;
	...
	void UbloxM8nUartInit(uint32_t baudrate);
