...
ReadUbloxM8n* ReadUbloxM8n::ubloxm8n_instance_handle_ = nullptr;
...
//ReadUbloxM8n read_ubloxm8n_task_instance_(&huart2);

ReadUbloxM8n::ReadUbloxM8n(UART_HandleTypeDef* huart):
TaskBase("UbloxM8nTask2", 1296, osPriorityAboveNormal),
gps_uart_(huart){

ubloxm8n_instance_handle_ = this;
 ...
 * @retval None
 */
void ReadUbloxM8n::UbloxM8nUartInit(uint32_t baudrate)
{
 ...
 */
void ReadUbloxM8n::SetBaudrate(const uint32_t baudrate){
	UbloxM8nUartInit(baudrate);
}

void ReadUbloxM8n::FlushUartDataRegister(){
 ...
 */
void ReadUbloxM8n::CalculateChecksum(uint8_t *buffer, uint16_t length, uint8_t *ck_a, uint8_t *ck_b) {
	*ck_a = 0;
	*ck_b = 0;
 ...
 */
bool ReadUbloxM8n::TxUartUbxPollCmd(const UbxMessage *message, uint32_t wait_time) {
 ...
 * @return true if successful, false otherwise
 */
bool ReadUbloxM8n::RxUartUbxPollMsgAck(uint8_t *payload_buffer, uint16_t payload_length, uint32_t wait_time){
 ...
 * @return true if successful, false otherwise
 */
bool ReadUbloxM8n::RxUartUbxPollMsg(uint8_t *payload_buffer, uint16_t payload_length, uint32_t wait_time){
	uint16_t rx_msg_length = 6 + payload_length + 2;
	HAL_StatusTypeDef status = HAL_UART_Receive(gps_uart_, rx_buffer_, rx_msg_length, wait_time);
	if(status != HAL_OK){
		DEBUG_PRINT("UART RX Failed: %d\n", status);
		return false;
	}
	...
	DEBUG_PRINT("Checksum Failed: Calculated: 0x%02X. 0x%02X -  Received: 0x%02X, 0x%02X\n", ck_a, ck_b, rx_buffer_[payload_length + 6],
			rx_buffer_[payload_length + 7]);
	return false;
}

bool ReadUbloxM8n::ResetGps(){
	UbloxM8nCfgRst ubx_cfg_rst_msg;
	...
	ubx_cfg_rst.length = sizeof(UbloxM8nCfgRst);
 ...
 * @return true if successful, false otherwise
 */
bool ReadUbloxM8n::UbxSaveCfg(uint32_t save_mask){
	UbloxM8nCfgCfg ubx_cfg_cfg;
	...
	ubx_cfg_cfg_msg.length = sizeof(UbloxM8nCfgCfg);
...
bool ReadUbloxM8n::GetVersion(){
	...
	uint8_t payload_buffer[160] = {0};
	// Send the UBX-MON-VER request
	TxUartUbxPollCmd(&ubx_mon_ver, 100);
	// Receive and process the response
	bool status = RxUartUbxPollMsg(payload_buffer, 160, 100);
	if(status){
		UbloxM8nVer *ublox_ver = (UbloxM8nVer *)(&payload_buffer[0]);
		DEBUG_PRINT("\r");
		DEBUG_PRINT("***********************************************\n");
 ...
 * @return true if successful, false otherwise
 */
bool ReadUbloxM8n::ConfigGpsUart1(const uint32_t baudrate){
	...
	UbloxM8nCfgPrt *ubx_cfg_prt_msg = (UbloxM8nCfgPrt *)(&payload_buffer[0]);
	...
	if(ubx_cfg_prt_msg->baudrate != baudrate ||
		...
		ubx_cfg_prt.length = sizeof(UbloxM8nCfgPrt);
...
//		}
		...
		ubx_cfg_prt_msg = (UbloxM8nCfgPrt *)(&payload_buffer[0]);
		if(status){
			DEBUG_PRINT("***********************************************\n");
...
bool ReadUbloxM8n::ConfigPrtProtocol(uint8_t port_id, uint16_t proto_mask){
	...
	TxUartUbxPollCmd(&ubx_cfg_prt, txrx_delay_ms_);
	// UBC-CFG-PRT payload length = 20
	uint8_t payload_buffer[sizeof(UbloxM8nCfgPrt)] = {0};
	bool status = RxUartUbxPollMsgAck(payload_buffer, 20, txrx_delay_ms_);

	UbloxM8nCfgPrt ubx_cfg_prt_data;
	memcpy(&ubx_cfg_prt_data, payload_buffer, sizeof(UbloxM8nCfgPrt));
	UbloxM8nCfgPrt* ubx_cfg_prt_msg = &ubx_cfg_prt_data;
...
bool ReadUbloxM8n::ConfigAuxPrts(){
...
bool ReadUbloxM8n::ConfigNav5(){
	...
	UbloxM8nCfgNav5 *ubx_cfg_nav5_msg = (UbloxM8nCfgNav5 *)(&payload_buffer);
	...
	if (ubx_cfg_nav5_msg->dyn_model != 8 ||
		...
		ubx_cfg_nav5.length = sizeof(UbloxM8nCfgNav5);
		...
		ubx_cfg_nav5_msg = (UbloxM8nCfgNav5 *)(&payload_buffer);
		if(status){
			DEBUG_PRINT("***********************************************\n");
...
bool ReadUbloxM8n::EnableNavPvtMsg(){
	UbxMessage ubx_cfg_msg;
	UbloxM8nCfgMsg ubx_cfg_msg_msg = {0};
	...
	ubx_cfg_msg.length = sizeof(UbloxM8nCfgMsg);
...
bool ReadUbloxM8n::DisableNavPvtMsg(){
	UbxMessage ubx_cfg_msg;
	UbloxM8nCfgMsg ubx_cfg_msg_msg = {0};
	ubx_cfg_msg_msg.msg_class = CLASS_NAV; // NAV class
	ubx_cfg_msg_msg.msg_id = ID_PVT;    // PVT message
...
void ReadUbloxM8n::GetCurrentBaudrate(){
...
bool ReadUbloxM8n::InitGps(uint32_t baudrate, uint16_t time_bw_samples_ms, uint8_t nav_rate){
	bool status;
	GetCurrentBaudrate();
	FlushUartDataRegister();
...
bool ReadUbloxM8n::ConfigGpsMeasRate(uint16_t time_bw_samples_ms, uint8_t nav_rate){
	...
	UbloxM8nCfgRate *ubx_cfg_rate_msg = (UbloxM8nCfgRate *)(&payload_buffer);
	...
	if (ubx_cfg_rate_msg->meas_rate != time_bw_samples_ms ||
		...
		// Create a CFG message to send to Ublox
		ubx_cfg_rate.length = sizeof(UbloxM8nCfgRate);
		...
		ubx_cfg_rate_msg = (UbloxM8nCfgRate *)(&payload_buffer);
		if(status){
			DEBUG_PRINT("***********************************************\n");
...
void ReadUbloxM8n::StartNavPvtMsg(){
	// Start UART reception in DMA Circular mode
	HAL_Delay(5);
	EnableNavPvtMsg();
	FlushUartDataRegister();
	HAL_UART_Receive_DMA(gps_uart_, nav_pvt_raw_buff_, sizeof(UbloxM8nNavPvt) + 6 + 2);
	HAL_Delay(5);
}

// Check if GPS has new NAV-PVT frame
bool ReadUbloxM8n::IsNavPvtFrameNew(){
	if(new_nav_pvt_frame_){
		// We have a new NAV-PVT frame set the flag to false for future check
		// and return true;
...
void ReadUbloxM8n::SetDataReadyFlag(){
	new_nav_pvt_frame_ = true;
}

void ReadUbloxM8n::ReceivedNewNavPvtFrame(){
	if (ubloxm8n_instance_handle_ != nullptr){
		ubloxm8n_instance_handle_->SetDataReadyFlag();
	}
}

void ReadUbloxM8n::ProcessNavPvtFrame(){
	uint8_t ck_a, ck_b;
	...
	memcpy(&nav_pvt_data_, &nav_pvt_raw_buff_[6], sizeof(UbloxM8nNavPvt));
...
void ReadUbloxM8n::Run() {
	Publisher<GpsData> ubloxm8n_pub(TopicID::UBLOXM9N);
	osDelay(100);
	vTaskSuspendAll();   // Suspend scheduler (no task preemption)
	InitGps(460800, 200, 1);
	StartNavPvtMsg();
	xTaskResumeAll();    // Resume scheduler (task switching enabled again)
...
//			used, highWaterMark * sizeof(StackType_t), 1296);
    	...
    	if(IsNavPvtFrameNew()){
    		ProcessNavPvtFrame();
    		ubloxm8n_pub.publish(gps_data_);
