...
RcSbus2* RcSbus2::rc_sbus2_instance_handle_ = nullptr;

extern UART_HandleTypeDef huart7;
//RcSbus2 read_rcinputs_task_instance_(&huart7);

RcSbus2::RcSbus2(UART_HandleTypeDef* huart):
TaskBase("ReadRcInTask", 864, osPriorityAboveNormal),
rc_uart_(huart){
	rc_sbus2_instance_handle_ = this;
}

void RcSbus2::RcSbusInit(){
//	FlushUartDataRegister(rc_uart_);
	HAL_UART_Receive_DMA(rc_uart_, rc_sbus2_ring_buffer_, RING_BUFFER_SIZE);
	HAL_Delay(100);
}

void RcSbus2::ReceivedNewSbusFrame(uint8_t id){
	if (rc_sbus2_instance_handle_ != nullptr){
		rc_sbus2_instance_handle_->RestartDma();
	}
}
	...
	while (read_pos_ != write_pos_) {
		if (rc_sbus2_ring_buffer_[read_pos_] == SBUS_START_BYTE) {
			...
			if (bytes_available >= FRAME_SIZE) {
				...
				if (rc_sbus2_ring_buffer_[frame_end] == SBUS_END_BYTE) {
					// Valid frame found, process it
					uint8_t payload_[23] = {0};
					for(size_t rIdx = 0; rIdx < (FRAME_SIZE - 2); rIdx++){
						payload_[rIdx] = rc_sbus2_ring_buffer_[(read_pos_ + 1 + rIdx) % RING_BUFFER_SIZE];
					}
...
void RcSbus2::RestartDma(){
	// Increment the write position by the number of received bytes
	write_pos_ = (RING_BUFFER_SIZE - __HAL_DMA_GET_COUNTER(rc_uart_->hdmarx)) % RING_BUFFER_SIZE;
	// Restart DMA reception for the next 50 bytes from the current position
//	HAL_UART_Receive_DMA(rc_uart_, rc_sbus2_ring_buffer_ + write_pos_, DMA_RX_SIZE);
//	HAL_GPIO_TogglePin(LED3_GPIO_Port, LED3_Pin);  // Toggle LED3
}
...
//        if (RcSbus2::rc_sbus2_instance_handle_ != nullptr) {
//            RcSbus2::ReceivedNewSbusFrame(0);  // ID currently unused
//        }
//    } else {
//        __NOP();
//    }
