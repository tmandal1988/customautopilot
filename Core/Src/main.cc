...
//  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_4, kMinPwmVal);
  ...
  /* USER CODE BEGIN 2 */
  bool sd_init = SetSdBus4BWide();//Set SD to 4B operation
  if(!sd_init){
	  ERROR_PRINT("Failed to set SD to 4B operation\n");
  }
  ...
  // Seed the authoritative parameter banks before any client task can run.
  parameters::ParameterStore::Instance().Initialize();
...
bool SetSdBus4BWide(){
	...
	sd_init &= (HAL_SD_Init(&hsd1) ==  HAL_OK);
	sd_init &= (HAL_SD_ConfigWideBusOperation(&hsd1, SDMMC_BUS_WIDE_4B) == HAL_OK);
...
void MPU_Config(void)
{
    ...
    /*** Region 0: Default - Deny all access to unmapped memory ***/
    MPU_InitStruct.Enable = MPU_REGION_ENABLE;
    MPU_InitStruct.Number = MPU_REGION_NUMBER0;
    MPU_InitStruct.BaseAddress = 0x0;
    MPU_InitStruct.Size = MPU_REGION_SIZE_4GB;
    MPU_InitStruct.SubRegionDisable = 0x87; // Disables subregions 0, 1, 7 (example)
    MPU_InitStruct.TypeExtField = MPU_TEX_LEVEL0;
    MPU_InitStruct.AccessPermission = MPU_REGION_NO_ACCESS;
    MPU_InitStruct.DisableExec = MPU_INSTRUCTION_ACCESS_DISABLE;
    MPU_InitStruct.IsShareable = MPU_ACCESS_SHAREABLE;
